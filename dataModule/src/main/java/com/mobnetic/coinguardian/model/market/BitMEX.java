...
public class BitMEX extends Market {
	...
	private final static String URL = "https://www.bitmex.com/api/v1/instrument?symbol=%1$s&columns=bidPrice,askPrice,volume24h,highPrice,lowPrice,lastPrice";
	...
	protected void parseCurrencyPairsFromJsonObject(int requestId, JSONObject jsonObject, List<CurrencyPairInfo> pairs) throws Exception {
		...
		for(int i = 0; i < pairNames.length(); ++i) {
			...
			String counter = null;
            try {
                base = fixCurrency(pairId.substring(0, 3));
                counter = fixCurrency(pairId.substring(3));
