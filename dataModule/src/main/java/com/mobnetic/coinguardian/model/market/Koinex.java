...
public class Koinex extends Market {
	...
	protected void parseTickerFromJsonObject(int requestId, JSONObject jsonObject, Ticker ticker, CheckerInfo checkerInfo) throws Exception {
		...
		final String tempCurrency = checkerInfo.getCurrencyCounter().toString();
		JSONObject tickerJsonObject = names.getJSONObject(tempCurrency);
	...
	protected void parseCurrencyPairsFromJsonObject(int requestId, JSONObject jsonObject, List<CurrencyPairInfo> pairs) throws Exception {
		...
		for(int i=0; i<currencyArray.length(); ++i) {
			String virtualCurrency = currencyArray.getString(i);
			if(virtualCurrency==null)
				continue;

			String currencyBase = "INR";
			String currencyCounter = virtualCurrency.toUpperCase(Locale.ENGLISH);
